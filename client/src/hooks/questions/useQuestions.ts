            ...
            prev.map((item) => {
                if ("options" in item)
                    return {
                        ...
                        options:
                            option.id === item.id
                                ? { ...item.options, label: option.label, icon: option.icon }
                                : item.options,
                    };
                return item;
            })
            ...
            prev.map((item) => {
                if ("options" in item) {
                    ...
                    return {
                        ...
                        options:
                            quesId === item.id
                                ? { ...item.options, arr: [...arr, "Option " + (arr.length + 1)] }
                                : item.options,
                    };
                }
                return item;
            ...
            prev.map((item) => {
                if ("options" in item) {
                    ...
                    return {
                        ...
                        options:
                            quesId === item.id
                                ? { ...item.options, arr: filterOptions(arr, optionId) }
                                : item.options,
                    };
                }
                return item;
                    ...
                    return {
                        ...
                        options:
                            quesId === item.id
                                ? {
                                      ...item.options,
                                      arr: optionArr.map((opt, index) =>
                                          index === optionId ? value : opt
                                      ),
                                  }
                                : item.options,
                    };
