    ...
    return (
        <div className="" tabIndex={-1} onBlur={() => setDisplayOptions(false)}>
            <div
                ...
                className="border border-gray-300 rounded-md py-3 px-4 flex items-center cursor-pointer"
            >
                <div className="mr-2">
                    <Icon icon={question.options.icon} size="2xl" />
                </div>
                <div className="mr-auto">{question.options.label}</div>
                <div className="-mt-2">
                    <Icon icon="sort-down" size="md" />
                </div>
            </div>
...
function DropdownOptions({ question, selectQuestion, toggleOptions }: OptionsProps) {
    return (
        <div
            style={{ top: "2.5rem" }}
            className={`fixed w-56 cursor-pointer bg-white shadow-md rounded-md z-50`}
        >
            {options.map((group, index) => (
                <ul className="border-b border-gray-300" key={index}>
                    {group.map((option) => (
                        <li
                            onClick={(e) => {
                                selectQuestion({ ...option, id: question.id });
                                toggleOptions(e);
                            }}
                            className={`my-2 py-1.5 px-4 flex items-center ${
                                question.options.label === option.label ? "bg-blue-50" : ""
                            } hover:bg-gray-200`}
                            key={option.label}
                        >
