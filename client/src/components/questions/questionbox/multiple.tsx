        ...
        <div>
            {arr.map((option, index) => (
                <div key={index} className="flex items-center mt-2">
                    <span className="mr-4">
                        {icon ? <Icon size="2xl" icon={icon} /> : `${index + 1}.`}
                    </span>
                    <Input
                        type="text"
                        textSize="md"
                        ...
                        handleChange={(e: React.ChangeEvent) =>
                            handleOptionChange(quesId, index, (e.target as HTMLInputElement).value)
                        }
                        placeholder={`Option ${index + 1}`}
                    />
                    <span
                        onClick={(e) => removeOption(quesId, index)}
                        className={`${arr.length > 1 ? "block" : "hidden"} ml-3`}
                    >
                        <Icon icon="times" />
                    </span>
                </div>
            ...
            <div className="mt-4 flex items-center">
                ...
                <p className=" ml-4 text-gray-500" onClick={(e) => addOption(quesId)}>
                    Add option
                </p>
            </div>
        </div>
